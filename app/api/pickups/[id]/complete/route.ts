...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { collected_kg, verification_photo_url, notes } = await req.json()

  const pickup = await prisma.pickup.findUnique({
    where: { id: parseInt(id) },
    include: { match: { include: { donation: true } } },
  })
  if (!pickup || !['arrived', 'en_route', 'accepted'].includes(pickup.status)) {
  ...
  const [updated] = await prisma.$transaction([
    prisma.pickup.update({
      ...
      data: {
        status: 'completed',
        collectedKg: collected_kg,
        verificationPhotoUrl: verification_photo_url,
        notes,
      },
    }),
    prisma.donation.update({
      where: { id: pickup.match.donationId },
